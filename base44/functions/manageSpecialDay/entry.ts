...
Deno.serve(async req => {
  try {
    ...
    const checkVersion = async dayId => {
      ...
      if (day.status !== "saved" && !["archive"].includes(body.action)) throw new Error("ניתן לערוך יום שמור בלבד");
      if (body.expected_updated_date && day.updated_date !== body.expected_updated_date) {
        const err = new Error("היום השתנה במקביל. יש לרענן ולנסות שוב."); err.code = "concurrent_change"; throw err;
      }
      return day;
    };
    ...
    const validate = async pack => {
      ...
      for (const a of assignments.filter(x => x.status !== "cancelled")) {
        ...
        const others = assignments.filter(x => x.id !== a.id), eligibility = dutyEligibility(teacher);
        teacherReason({ teacher, day, slot, position, ...ctx, current: others }).forEach(message => { const critical = !eligibility.eligible && message === eligibility.reason; errors.push(issue(critical ? "critical_ineligible_teacher" : "teacher_unavailable", `${teacher.full_name}: ${message}`, { assignment_id: a.id, teacher_id: teacher.id, critical })); });
      }
    ...
    if (body.action === "create") {
      ...
      const day = await e.SpecialDay.create({ name: data.name, type: data.type || "other", date: data.date, description: data.description || "", instructions: data.instructions || "", status: "saved", replace_regular_schedule: data.replace_regular_schedule !== false, allow_regular_overlap: false, respect_regular_schedule: data.respect_regular_schedule !== false, quota_mode: data.quota_mode || "full", allow_multiple_shifts: !!data.allow_multiple_shifts, max_shifts_per_teacher: data.max_shifts_per_teacher || 1, morning_reminder_time: data.morning_reminder_time || "08:00", pre_shift_reminder_minutes: data.pre_shift_reminder_minutes ?? 10, version: 1, created_by_id: user.id, created_by_name: actor.full_name });
      let source = null;
    ...
    if (body.action === "assign") { const pack = await loadPack(day.id), slot = pack.slots.find(s => s.id === body.time_slot_id), position = pack.positions.find(p => p.id === body.position_id), ctx = await context(day), teacher = ctx.teachers.find(t => t.id === body.teacher_id); if (!slot || !position || !teacher) return Response.json({ error: "נתוני השיבוץ חסרים" }, { status: 400 }); const eligibility = dutyEligibility(teacher); if (!eligibility.eligible) return Response.json({ error: eligibility.reason, critical: true }, { status: 422 }); const reasons = teacherReason({ teacher, day, slot, position, ...ctx, current: pack.assignments }); if (reasons.length) return Response.json({ error: reasons.join("; "), reasons }, { status: 409 }); await e.SpecialAssignment.create({ special_day_id: day.id, special_day_name: day.name, date: day.date, time_slot_id: slot.id, time_slot_name: slot.name, start_time: slot.start_time, end_time: slot.end_time, position_id: position.id, position_name: position.name, position_instructions: [day.instructions, position.instructions].filter(Boolean).join("\n"), teacher_id: teacher.id, teacher_name: teacher.full_name, status: "scheduled", source: body.source || "manual", plan_status: "saved" }); await snapshot(day.id, "assign_teacher"); return Response.json(await loadPack(day.id)); }
    ...
    if (body.action === "restore") { const revision = await e.SpecialDayRevision.get(body.revision_id); if (!revision || revision.special_day_id !== day.id) return Response.json({ error: "גרסה לא נמצאה" }, { status: 404 }); const snap = revision.snapshot; const restoreIds = [...new Set(snap.assignments.map(a => a.teacher_id).filter(Boolean))], restoreTeachers = restoreIds.length ? await e.TeacherProfile.filter({ id: { $in: restoreIds } }) : [], blockedRestore = restoreTeachers.find(t => !dutyEligibility(t).eligible); if (blockedRestore) return Response.json({ error: dutyEligibility(blockedRestore).reason, critical: true }, { status: 422 }); await Promise.all([e.SpecialAssignment.deleteMany({ special_day_id: day.id }), e.SpecialPositionRequirement.deleteMany({ special_day_id: day.id }), e.SpecialTimeSlot.deleteMany({ special_day_id: day.id }), e.SpecialPosition.deleteMany({ special_day_id: day.id })]); const slots = snap.slots.length ? await e.SpecialTimeSlot.bulkCreate(snap.slots.map(({ id, ...s }) => ({ ...s, special_day_id: day.id }))) : [], positions = snap.positions.length ? await e.SpecialPosition.bulkCreate(snap.positions.map(({ id, ...p }) => ({ ...p, special_day_id: day.id }))) : []; const sm = new Map(snap.slots.map((s, i) => [s.id, slots[i]?.id])), pm = new Map(snap.positions.map((p, i) => [p.id, positions[i]?.id])); if (snap.requirements.length) await e.SpecialPositionRequirement.bulkCreate(snap.requirements.map(({ id, ...r }) => ({ ...r, special_day_id: day.id, time_slot_id: sm.get(r.time_slot_id), position_id: pm.get(r.position_id) }))); if (snap.assignments.length) await e.SpecialAssignment.bulkCreate(snap.assignments.map(({ id, ...a }) => ({ ...a, special_day_id: day.id, time_slot_id: sm.get(a.time_slot_id), position_id: pm.get(a.position_id), plan_status: "saved" }))); await e.SpecialDay.update(day.id, { name: snap.day.name, type: snap.day.type, date: snap.day.date, description: snap.day.description || "", instructions: snap.day.instructions || "", status: "saved" }); await snapshot(day.id, "restore_version"); return Response.json(await loadPack(day.id)); }
