...
const keyOf = item => `${item.day_of_week}|${item.break_type}|${item.station_id}`;
const unique = values => [...new Set((values || []).filter(Boolean))];
const quotaFor = (teacher, rules) => { const rule = rules.find(r => (teacher.weekly_teaching_hours || 0) >= r.min_hours && (teacher.weekly_teaching_hours || 0) <= r.max_hours); return rule ? (rule.big_count || 0) + (rule.medium_count || 0) + (rule.small_count || 0) : 0; };
const nextDate = day => { const date = new Date(); const add = (day - date.getDay() + 7) % 7; date.setDate(date.getDate() + add); return date.toISOString().slice(0, 10); };
...
Deno.serve(async req => {
  try {
    ...
    } else if (body.action === "clear_day") assignments = assignments.filter(item => item.day_of_week !== Number(body.day_of_week));
...
function makeSlot(body, stations, breaks) {
...
function candidate(teacher, slot, data, ignoreKey) {
  if (!teacher) return { available: false, reasons: ["המורה לא נמצא"], warnings: [], score: -999 };
  const reasons = [], warnings = [], date = nextDate(slot.day_of_week), schedule = data.schedules.filter(s => s.teacher_id === teacher.id && s.day_of_week === slot.day_of_week), assigned = data.assignments.filter(a => keyOf(a) !== ignoreKey && a.teacher_ids?.includes(teacher.id));
...
function autoAssign(data) {
  const assignments = [], sortedBreaks = [...data.breaks].sort((a, b) => mins(a.start_time) - mins(b.start_time));
  for (let day = 0; day <= 4; day++) for (const brk of sortedBreaks.filter(b => b.active_days?.includes(day))) for (const station of data.stations.filter(s => s.active_break_types?.includes(brk.break_type))) {
    const slot = makeSlot({ day_of_week: day, break_type: brk.break_type, station_id: station.id }, data.stations, data.breaks), ids = [];
    for (let index = 0; index < slot.required; index++) { const live = { ...data, assignments: [...assignments, { ...slot, teacher_ids: ids }] }, best = data.teachers.map(t => candidate(t, slot, live, keyOf(slot))).filter(c => c.available && !ids.includes(c.id)).sort((a, b) => b.score - a.score)[0]; if (best) ids.push(best.id); }
    if (ids.length) assignments.push({ day_of_week: day, break_type: brk.break_type, station_id: station.id, teacher_ids: ids, teacher_names: ids.map(id => data.teachers.find(t => t.id === id)?.full_name || ""), override_reason: "שיבוץ אוטומטי" });
  }
  return assignments;
}
