...
export default function TeacherRow({ teacher, onOpen, onDelete }) {
  ...
  return (
    <div
      ...
      className="flex w-full cursor-pointer items-center gap-3 rounded-xl border border-border bg-card p-3 shadow-sm transition-colors last:border-b-0 hover:bg-muted/50"
    >
      <div className="relative shrink-0">
        <div className={`flex h-12 w-12 items-center justify-center rounded-lg text-base font-bold ${tag.block}`}>
          {initialsOf(teacher.full_name)}
        </div>
        <span className={`absolute -top-1.5 right-1 rounded-full px-1.5 py-0.5 text-[10px] font-semibold leading-none shadow-sm ${tag.badge}`}>
          {tag.label}
        </span>
      </div>
