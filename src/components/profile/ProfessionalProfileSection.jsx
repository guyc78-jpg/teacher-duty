  ...
  return (
    <section className="rounded-xl border border-border bg-card px-4">
      <div className="flex min-h-14 items-center justify-between gap-3 border-b border-border py-2">
        ...
        {!editing && !pendingRequest && <Button size="sm" variant="outline" onClick={() => setEditing(true)}><Edit className="h-3.5 w-3.5" />בקשת שינוי</Button>}
      </div>
      {pendingRequest && (
      ...
      {editing ? <ChangeRequestForm teacher={teacher} onCancel={() => setEditing(false)} onSubmitted={onSubmitted} /> : (
        <>
          ...
          <p className="border-t border-border py-3 text-xs text-muted-foreground">שינוי מקצוע, חינוך, ימי חופש או פטור ייכנס לתוקף רק לאחר אישור מנהל.</p>
        </>
      )}
    </section>
  );
