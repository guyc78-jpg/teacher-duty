...
export default function FixedScheduleToolbar({ day, setDay, busy, onAuto, onClear, onImport, onExport }) {
  ...
  return <div className="space-y-3"><div className="grid grid-cols-5 gap-1 rounded-xl bg-muted p-1">{days.map((label, index) => <button key={label} onClick={() => setDay(index)} className={`min-h-11 rounded-lg text-sm font-medium ${day === index ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground"}`}>{label}</button>)}</div><div className="grid grid-cols-2 gap-2"><Button variant="outline" className={actionClass} disabled={busy} onClick={onAuto}><Sparkles />שיבוץ אוטומטי</Button><Button variant="outline" className={actionClass} disabled={busy} onClick={onExport}><Download />ייצוא נתונים</Button><Button variant="outline" className={actionClass} disabled={busy} onClick={() => fileInput.current?.click()}><Upload />ייבוא נתונים</Button><Button variant="outline" className="h-14 w-full justify-between rounded-xl border-destructive/40 bg-card px-4 text-base font-semibold text-destructive shadow-sm hover:bg-destructive/10 hover:text-destructive" disabled={busy} onClick={onClear}><Eraser />נקה יום</Button><input ref={fileInput} type="file" accept="application/json,.json" className="hidden" onChange={chooseFile} /></div></div>;
}