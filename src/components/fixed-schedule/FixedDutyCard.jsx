...
function FixedDutyCard({ day, station, assignment, breakType, onClick }) {
  const required = station.staffing_requirements?.[breakType] || 1, names = assignment?.teacher_names || [], full = names.length >= required;
  return <button id={`fixed-slot-${day}-${breakType}-${station.id}`} onClick={onClick} className={`w-full rounded-xl border p-3 text-right transition-colors hover:border-primary ${full ? "border-success/30 bg-success/5" : "border-destructive/30 bg-destructive/5"}`}>
    ...
    {names.length ? <div className="mt-2 flex flex-wrap gap-1">{(assignment?.teacher_ids || []).map((id, index) => <span key={id} className={`rounded-full border px-2 py-0.5 text-xs font-medium text-foreground ${teacherColor(id)}`}>{names[index] || ""}</span>)}</div> : <p className="mt-2 text-sm text-destructive">טרם שובצו מורים</p>}
  </button>;
}
