...
export default function WeeklyScheduleView({ dates, assignments, summaries, onOpenDay }) {
  const [expanded, setExpanded] = useState("");
  return <div className="space-y-2">{dates.map(date => { const day = summaries[date]; const open = expanded === date; return <section key={date} className="rounded-xl border border-border bg-card">
    <div className="flex items-center gap-2 p-3"><button className="min-w-0 flex-1 text-right" onClick={() => onOpenDay(date)}><h3 className="font-bold">{heDate(date)}</h3><p className="mt-1 text-xs text-muted-foreground">{day.assignments} שיבוצים · {day.missing} חסרים · {day.conflicts} התנגשויות · {day.warnings} חריגות</p></button><span className={`rounded-full px-2 py-1 text-xs font-semibold ${day.statusClass}`}>{day.status}</span><button className="hidden min-h-11 min-w-11 items-center justify-center lg:flex" aria-label="הרחבת יום" onClick={() => setExpanded(open ? "" : date)}>{open ? <ChevronUp /> : <ChevronDown />}</button></div>
