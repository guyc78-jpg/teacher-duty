...
export default function ScheduleEditor() {
  ...
  const load = useCallback(async () => {
    ...
    if (current && isManagement(current)) {
      ...
      const chosen = plan ? allPlans.find(item => item.id === plan.id) : allPlans.find(item => item.status === "saved") || allPlans[0]; if (chosen) await loadPlan(chosen);
    }
    setLoading(false);
  }, [loadPlan, plan]);
  useEffect(() => { load(); }, []);
  ...
  const generate = async () => { setBusy(true); setMessage(""); try { const start = weekStart(iso(new Date())), end = fromIso(start); end.setDate(end.getDate() + 32); const result = await generateDutyDraft({ start_date: start, end_date: iso(end), plan_name: `לוח ${start}`, approve_incomplete: true }); setMessage(`נוצרו ונשמרו ${result.data.assignments_created} שיבוצים`); await load(); } catch (error) { setMessage(error.response?.data?.error || error.message); } finally { setBusy(false); } };
  ...
  return <div className="space-y-3 pb-4"><ScheduleHeader view={view} setView={setView} date={date} onDate={value => { if (schoolDate(fromIso(value))) { setDate(value); setWeek(weekStart(value)); } else setMessage("ניתן להציג רק ימים ראשון–חמישי"); }} onPrevious={() => view === "day" ? setDate(moveSchoolDay(date, -1)) : setWeek(iso(new Date(fromIso(week).setDate(fromIso(week).getDate() - 7))))} onNext={() => view === "day" ? setDate(moveSchoolDay(date, 1)) : setWeek(iso(new Date(fromIso(week).setDate(fromIso(week).getDate() + 7))))} onToday={() => { const today = iso(new Date()); const safe = schoolDate(fromIso(today)) ? today : moveSchoolDay(today, 1); setDate(safe); setWeek(weekStart(safe)); }} />
    <div className="flex flex-wrap gap-2"><select aria-label="בחירת תוכנית" value={plan?.id || ""} onChange={e => loadPlan(plans.find(item => item.id === e.target.value))} className="h-10 min-w-0 basis-full rounded-lg border border-input bg-background px-3 text-sm sm:flex-1 sm:basis-0"><option value="">בחר תוכנית</option>{plans.map(item => <option key={item.id} value={item.id}>{item.name} · גרסה {item.version} · {item.status === "saved" ? "שמור" : "פורסם"}</option>)}</select><Button variant="outline" onClick={generate} disabled={busy}><Plus />שיבוץ אוטומטי</Button>{plan?.status === "saved" && plan.version > 1 && <Button variant="outline" onClick={restore} disabled={busy}><History />שחזור</Button>}{plan?.status === "saved" && <Button onClick={openReview} disabled={busy}><Send />בדיקה ופרסום</Button>}</div>
