...
export default function FixedSchedule() {
  ...
  const openReview = async () => { setBusy(true); setMessage(""); try { const r = await manageFixedSchedule({ action: "validate", expected_updated_date: data.template.updated_date }); setData(current => ({ ...current, validation: r.data })); setReview(true); } catch (e) { setMessage(e.response?.data?.error || e.message); } finally { setBusy(false); } };
